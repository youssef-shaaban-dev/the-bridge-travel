    ...
    return (
        <div className="flex flex-col xl:flex-row gap-3 md:gap-4 mb-12 h-auto">
            ...
            <button
                ...
                aria-label="View main tour image in full screen"
                className="xl:w-[66%] relative aspect-[4/3] sm:aspect-[16/9] md:h-[450px] xl:h-[600px] overflow-hidden rounded-2xl md:rounded-3xl group cursor-pointer shadow-lg shadow-slate-200/50 block w-full border-none p-0"
            >
                <img
                    src={images[0]}
                    alt="Main tour view"
            ...
            <div className="xl:w-[34%] grid grid-cols-2 xl:flex xl:flex-col gap-3 md:gap-4 xl:h-[600px]">
                {images.slice(1).map((img, idx) => (
                    <button
                        ...
                        aria-label={`View gallery image ${idx + 2}`}
                        className={`relative overflow-hidden rounded-xl md:rounded-2xl group cursor-pointer shadow-md shadow-slate-100/50 xl:flex-1 border-none p-0 text-left ${idx === 2
                    ...
                    >
                        <img
                            src={img}
                            alt={`Tour view perspective ${idx + 2}`}
                            width={400}
                            height={300}
                        ...
                        {idx === 2 && (
                            <div
                                ...
                                className="absolute bottom-4 right-4 bg-white/90 backdrop-blur-sm text-slate-900 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-xl hover:bg-white transition-all transform active:scale-95 z-20 cursor-pointer"
                            >
                                <Grid className="h-4 w-4" aria-hidden="true" />
                                <span>Gallery</span>
                            </div>
