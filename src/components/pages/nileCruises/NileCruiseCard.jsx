    ...
    return (
        ...
        >
            ...
            <div className="relative h-64 overflow-hidden">
                <Link to={`/nile-cruises/${cruise.slug}`} aria-label={`View itinerary for ${cruise.title}`}>
                    <img
                        src={cruise.images[0]}
                        alt={`Nile Cruise: ${cruise.title}`}
                        width={400}
                        height={256}
            ...
            <div className="p-6 flex flex-col flex-1">
                ...
                <div className="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between">
                    <Link to={`/nile-cruises/${cruise.slug}`} aria-label={`View itinerary for ${cruise.title}`} className="text-[#22455C] font-bold text-sm flex items-center gap-2 hover:text-[#BC8B22] transition-colors relative group/btn overflow-hidden">
                        View Itinerary
                        <ArrowRight className="h-4 w-4 transition-transform group-hover/btn:translate-x-1" aria-hidden="true" />
                    </Link>

                    <Link to={`/nile-cruises/${cruise.slug}`} aria-label={`Book the ${cruise.title} cruise now`}>
